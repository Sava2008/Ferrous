...
const TIME_CHECK_NODES_OFFSET: u64 = 2500; // how often to check for timeout
...
impl Engine {
        ...
        *node_count += 1;
        ...
        if !in_check {
            if maximizing {
                ...
                if stand_pat + QUEEN_VALUE + QUIESCENCE_DELTA < alpha {
                    self.transposition_table.record_entry(
                        &self.current_hash,
                        TTEntry {
                            hash: self.current_hash,
                ...
                if stand_pat - QUEEN_VALUE - QUIESCENCE_DELTA > beta {
                    self.transposition_table.record_entry(
                        &self.current_hash,
                        TTEntry {
                            hash: self.current_hash,
    ...
    pub fn find_best_move(
        ...
        self.prepare_before_search(&copied_board);

        self.evaluate(&copied_board);
