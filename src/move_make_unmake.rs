...
impl Board {
    ...
    fn perform_capture(
        ...
        let (bitboard_for_capture, occupancy): (&mut u64, &mut u64) = if color == 16 {
            match enemy {
                WHITE_BISHOP_U32 => {
            ...
            4 => {
                if color == 8 {
                    ...
                    *&mut self.white_queens |= end;
                    self.cached_pieces[to_sq as usize] = Some(WHITE_QUEEN_U32);
                } else {
                    *&mut self.black_pawns &= start;
                    *&mut self.black_queens |= end;
                    self.cached_pieces[to_sq as usize] = Some(BLACK_QUEEN_U32);
                }
            }
            3 => {
                if color == 8 {
                    ...
                    *&mut self.white_rooks |= end;
                    self.cached_pieces[to_sq as usize] = Some(WHITE_ROOK_U32);
                } else {
                    *&mut self.black_pawns &= start;
                    *&mut self.black_rooks |= end;
                    self.cached_pieces[to_sq as usize] = Some(BLACK_ROOK_U32);
                }
            }
            2 => {
                if color == 8 {
                    ...
                    *&mut self.white_bishops |= end;
                    self.cached_pieces[to_sq as usize] = Some(WHITE_BISHOP_U32);
                } else {
                    *&mut self.black_pawns &= start;
                    *&mut self.black_bishops |= end;
                    self.cached_pieces[to_sq as usize] = Some(BLACK_BISHOP_U32);
                }
            }
            1 => {
                if color == 8 {
                    ...
                    *&mut self.white_knights |= end;
                    self.cached_pieces[to_sq as usize] = Some(WHITE_KNIGHT_U32);
                } else {
                    *&mut self.black_pawns &= start;
                    *&mut self.black_knights |= end;
                    self.cached_pieces[to_sq as usize] = Some(BLACK_KNIGHT_U32);
                }
            }
