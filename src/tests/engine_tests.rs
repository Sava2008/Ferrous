...
fn checkmate_in_two_test1() -> () {
    ...
    assert_eq!(from, 14);
    assert_eq!(to, 6);
    board.perform_move(engine_move, &mut state, 16);
    let response: u32 = opponent_engine.find_best_move(&board, &mut state).unwrap();
    board.perform_move(response, &mut state, 8);
...
fn checkmate_in_three_test1() -> () {
    ...
    assert_eq!(from, 30);
    assert_eq!(to, 22);
    board.perform_move(engine_move2, &mut state, 16);
...
fn checkmate_in_four_test1() -> () {
    ...
    assert_eq!(to, 4);
    println!("{from}: {to}");
    board.perform_move(engine_move3, &mut state, 16);
